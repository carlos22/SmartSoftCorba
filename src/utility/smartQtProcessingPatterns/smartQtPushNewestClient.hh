...
/**
  A decorator class for the CHS::PushNewestClient
 ...
 */
template<class Data>
class QtPushNewestClient: 
  public CHS::SmartTask, 
  public CHS::PushNewestClient<Data>,
  public QtSignallingInterface
{
public:
  QtPushNewestClient(CHS::SmartComponent *component, const std::string &port, CHS::WiringSlave* slave, unsigned int client_id = 0) throw(CHS::SmartError)
  : CHS::PushNewestClient<Data>(component, port, slave),
  ...
  QtPushNewestClient(CHS::SmartComponent *component, unsigned int client_id = 0) throw(CHS::SmartError)
  : CHS::PushNewestClient<Data>(component),
    QtSignallingInterface(client_id),
    _state_mutex(),
    _state_condition(_state_mutex)
  {
  ...
  QtPushNewestClient(CHS::SmartComponent *component, const std::string &server, const std::string &service, unsigned int client_id = 0) throw(CHS::SmartError)
  : CHS::PushNewestClient<Data>(component,server,service),
    QtSignallingInterface(client_id),
    _state_mutex(),
    _state_condition(_state_mutex)
  {
  ...
  int svc()
  {
    ...
    while(!_state.cancel)
    {
      if(_state.subscribed)
      {
        ...
        status = CHS::PushNewestClient<Data>::getUpdateWait(dummy);
   ...
   */
  inline CHS::StatusCode subscribe() throw()
  {
    CHS::StatusCode status = CHS::PushNewestClient<Data>::subscribe();
  ...
  {
    std::cerr << "Warning: Smart::QtPushNewestClient::getUpdateWait() called." << std::endl;
