...
#include "SampleStateServer.hh"

// constructor
SampleStateServer::SampleStateServer()
{
	std::cout << "constructor SampleStateServer\n";

	ini.stateServer.serviceName = "state";
}

void SampleStateServer::init(int argc, char *argv[])
{
	try
	{
		component = new CHS::SmartComponent("SampleStateServer", argc, argv);
		loadParameter(argc, argv);

		// create ports
		stateServer = new CHS::SmartStateServer(component,
				ini.stateServer.serviceName, stateChange);
		// define states for stateServer (CHS::SmartStateServer) 
		if (stateServer->defineStates("mainActive", "subActive")
				!= CHS::SMART_OK)
...
void SampleStateServer::loadParameter(int argc, char *argv[])
{
	/* 
	...
	try
	{
		...
		{
			std::cout << "load SampleStateServer.ini parameter file\n";
			parameter.addFile("SampleStateServer.ini");
		}
