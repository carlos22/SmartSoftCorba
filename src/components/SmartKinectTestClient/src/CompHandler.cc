...
void CompHandler::onStartup()
{
	...
	std::cout << "connecting to: "
			<< COMP->ini.imagePushNewestClient.serverName << "; "
			<< COMP->ini.imagePushNewestClient.serviceName << std::endl;
	status = COMP->imagePushNewestClient->connect(
			COMP->ini.imagePushNewestClient.serverName,
	...
	std::cout << "connecting to: " << COMP->ini.kinectQueryClient.serverName
			<< "; " << COMP->ini.kinectQueryClient.serviceName << std::endl;
	status = COMP->kinectQueryClient->connect(
			COMP->ini.kinectQueryClient.serverName,
			COMP->ini.kinectQueryClient.serviceName);
	while (status != CHS::SMART_OK)
	{
		usleep(500000);
		status = COMP->kinectQueryClient->connect(
