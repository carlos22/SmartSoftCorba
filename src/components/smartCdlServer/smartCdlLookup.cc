...
//
  ...
  for (vi=ivmin;vi<=ivmax;vi++) {
    for (wi=iwmin;wi<=iwmax;wi++) {
      ...
      dist   = dist -abs00(v)*deltat;if (dist  < 0.0) dist =0.0;
      angle  = angle-abs00(w)*deltat;if (angle < 0.0) angle=0.0;
...
//            wsoll = sqrt(2*goalHeadingRelative*wmax);
            ...
            if (maxDistance < 200.0) vsoll=0.0; else vsoll=sqrt(2*maxDistance*amax);
