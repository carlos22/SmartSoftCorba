...
bool LaserTask::accquireNewScan(CommBasicObjects::CommMobileLaserScan& scan) {
	...
	if (laser.read_laser_data()) {
		...
		scan.set_scan_double_field_of_view(-0.5 * (COMP->ini.scanner.opening_angle / 100.0) + laser.get_resolution(),
				laser.get_resolution());
		...
		const int desiredScans = COMP->ini.scanner.opening_angle / (laser.get_resolution() * 100.0);
