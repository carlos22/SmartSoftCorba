...
void PushTimedHandler::handlePushTimer(CHS::PushTimedServer<CommBasicObjects::CommMobileLaserScan> & server) throw() {

	if (COMP->ini.scanner.verbose)
		std::cout << "send scan via push timed\n";

	COMP->ScanLock.acquire();
	server.put(COMP->global_scan);
