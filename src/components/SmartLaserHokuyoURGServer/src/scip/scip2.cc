...
int switchToScip2(tPort* aPort)
{
  ...
  SCIP_TRACE("Trying to switch to SCIP2.0 mode ...\n");

  if(scipProtCheck(aPort,2)){
    SCIP_TRACE("The sensor is already in SCIP2 mode\n\n");
    return 0;
  }
  ...
  SCIP_TRACE("Switched to SCIP2.0 mode\n\n");
...
int scip2ShowVersion(tPort* aPort)
{
  ...
  SCIP_TRACE("Showing sensor version ...\n");
...
int scip2ShowParameter(tPort* aPort)
{
  ...
  SCIP_TRACE("Showing sensor parameters ...\n");
...
int scip2ShowStatus(tPort* aPort)
{
  ...
  SCIP_TRACE("Showing sensor status ...\n");
...
int scip2ShowTime(tPort* aPort)
{
  ...
  SCIP_TRACE("Showing host time and sensor time ...\n");
...
int scip2SetComSpeed(tPort* aPort, int aSpeed)
{
  ...
  ret=scipSendCmd(aPort,cmd,2);
#ifdef DEBUG
  SCIP_TRACE("ret = %d",ret);
#endif
  // Ad-Hoc、Main Correction
  if(ret==3){  // Status "3", Already running on the set speed
    SCIP_TRACE("The sensor is already worked at %d bps\n",aSpeed);
    SCIP_TRACE("The sensor would be connected on USB\n\n");
    return 0;
  }
  if(ret!=0)
    return ret;
  ...
  if(getBitrate(fileno(aPort->dev))==speed){
    ...
    SCIP_TRACE("Could not set speed : real bitrate = %d [speed_t]\n",getBitrate(fileno(aPort->dev)));
    return -3;
  }
}
  ...
  for(i=0;i<aScanNum;++i){
    *encData='\0';
    ...
    if(strcmp(line,SCIP2_MEASURE_STATUS)!=0){
      //invalid
      SCIP_TRACE("STATUS CHECK FAILED : %s",line);
      skipReceiveBuf(aPort->dev);
      for(j=0;j<aScanNum;++j)
        free(scan[j]);
      free(scan);
...
int* scip2GetScan(tPort* aPort, int aStartStep, int aEndStep, int aStepCluster, EncType_t aEncType, int *aStepNum)
{
  ...
  if(aPort->state!=STATE_MEASURE_STORE){
    SCIP_TRACE("Can not get a scan (Laser OFF, No stored scan)\n");
    return NULL;
  }
