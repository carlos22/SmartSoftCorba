...
void skipReceiveBuf(FILE *aDev)
{
  ...
  while(fgets(line,sizeof(line),aDev)!=NULL)
    ...
    {
      SCIP_TRACE("skipping receive buffer : %s\n", line);
    }
}
...
*/
tPort* openPort(const char *aDevName, int aSpeed)
{
  tPort* port;
  FILE* dev;
...
int scipSendCmd(tPort* aPort, char *aCmd, int aProtocol)
{
  ...
  if(!scipProtCheck(aPort,aProtocol)){
    SCIP_TRACE("INVALID VERSION SCIP COMMAND :\tThe sensor is not work in SCIP%d mode\n",aProtocol);
    return -2;
  }

  i=fwrite(aCmd,sizeof(char),strlen(aCmd),aPort->dev);
  if(i==0){
    SCIP_TRACE("Could not send command ");
    return -1;
  }
  ...
  if(strcmp(line,aCmd)!=0){
    // invalid
    SCIP_TRACE("INVALID ECHO BACK :\tExp = %s\tRec = %s\n",aCmd,line);
...
// #ifdef DEBUG
#if 1
  SCIP_TRACE(stdout,"status = %s",line);
#endif
...
int scipReceiveResEnd(tPort* aPort)
{
  ...
  if(strcmp(line,LF_STRING)!=0){  // invalid
    SCIP_TRACE("This response is not the end code\n");
    skipReceiveBuf(aPort->dev);
    return -2;
  }
  return 0;
