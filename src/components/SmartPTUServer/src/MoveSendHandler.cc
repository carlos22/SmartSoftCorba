...
void MoveSendHandler::handleSend(const CommPTUObjects::CommPTUMoveRequest &r) throw()
{
	CommPTUObjects::CommPTUMoveResponse answer;
	CommPTUObjects::PTUGoalEventState eventState;

	if (CHS::SMART_OK == COMP->stateServer->tryAcquire("active"))
	{
		COMP->stateServer->release("active");

		eventState.set_state(CommPTUObjects::PTUMoveStatus::GOAL_NOT_REACHED);
		COMP->goalEventServer->put(eventState);

		// move ptu
		COMP->ptu.move(r, answer);
