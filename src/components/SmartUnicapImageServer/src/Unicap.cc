...
void Unicap::init_getCameraAndOpenCamera() {
	...
	for (; SUCCESS(status) && (dev_count < MAX_DEVICES); dev_count++) {
		...
		if (SUCCESS(status)) {
			...
			if (strcmp(devices[dev_count].device, COMP->ini.hardware.device.c_str()) == 0) {
				// Get first camera on the device
				if (dev == -1) {
					dev = dev_count;
				}
				// Get the camera corresponding to the identifier
				if (strcmp(devices[dev_count].identifier, COMP->ini.hardware.identifier.c_str()) == 0) {
					dev = dev_count;
				}
			}
		} else {
...
void Unicap::init_getAndSetPropertiesOfCamera() {
		...
		if (SUCCESS(status)) {
			if (properties[range_ppty_count].type == UNICAP_PROPERTY_TYPE_RANGE || properties[range_ppty_count].type
				...
				if (strcmp(properties[range_ppty_count].identifier, "auto_exposure") == 0) {
					...
					_handleProperty(COMP->ini.hardware_properties.auto_exposure, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "white_balance_mode") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: white_balance_mode\n";
					_handleProperty(COMP->ini.hardware_properties.white_balance_mode, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "brightness") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: brightness\n";
					_handleProperty(COMP->ini.hardware_properties.brightness, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "gain") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: gain\n";
					_handleProperty(COMP->ini.hardware_properties.gain, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "gamma") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: gamma\n";
					_handleProperty(COMP->ini.hardware_properties.gamma, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "hue") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: hue\n";
					_handleProperty(COMP->ini.hardware_properties.hue, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "saturation") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: saturation\n";
					_handleProperty(COMP->ini.hardware_properties.saturation, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "sharpness") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: sharpness\n";
					_handleProperty(COMP->ini.hardware_properties.sharpness, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "shutter") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: shutter\n";
					_handleProperty(COMP->ini.hardware_properties.shutter, properties[range_ppty_count]);
				}
				// Set only if Auto_white_balance is false
					...
					_handleProperty(COMP->ini.hardware_properties.white_balance_u, properties[range_ppty_count]);
				}
				// Set only if Auto_white_balance is false
					...
					_handleProperty(COMP->ini.hardware_properties.white_balance_v, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "trigger_mode") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: trigger_mode\n";
					_handleMenuProperty(COMP->ini.hardware_properties.trigger_mode, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "trigger_polarity") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: trigger_polarity\n";
					_handleMenuProperty(COMP->ini.hardware_properties.trigger_polarity, properties[range_ppty_count]);
				} else if (strcmp(properties[range_ppty_count].identifier, "frame rate") == 0) {
					if (COMP->ini.hardware.debug_info)
						std::cout << "Prop: frame rate\n";
					_handleProperty(COMP->ini.hardware_properties.framerate, properties[range_ppty_count]);
				} else {
					range_ppty_count++;
					continue;
				}
	...
	// Set auto mode
	if (COMP->ini.hardware_properties.autoflag_white_balance_mode) {
		unicap_set_property_auto(_handle, "white_balance_mode");
	}
	if (COMP->ini.hardware_properties.autoflag_shutter) {
...
void Unicap::getImage(CommVisionObjects::CommVideoImage& image) {
