...
//	ifstream file("lastPos");
//	string line;
//	int counter = 0;
//
//	if (file.is_open())
//	{
//		while (!file.eof() && counter < 16)
//		{
//			getline(file, line);
//			_matrix[counter] = parseDouble(line);
//			cout << _matrix[counter] << endl;
//
//			counter++;
//		}
//	}
//	else
//	{
//		cout << "lastPos couldn't be read" << endl;
//	}
//	file.close();
//
...
static void zprPick(GLdouble x, GLdouble y, GLdouble delX, GLdouble delY)
{
	...
	if (hits != 0)
	{
		...
		{
			printf("\tsize = %u, min = %u, max = %u : ", buffer[j], buffer[j
					+ 1], buffer[j + 2]);
