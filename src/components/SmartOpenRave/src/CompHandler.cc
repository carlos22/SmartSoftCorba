...
void CompHandler::onStartup() {
	...
	if (COMP->ini.PortParameter.withObjectRecognition) {
		...
		std::cout << "connecting to: " << COMP->ini.environementQueryClient.serverName << "; "
				<< COMP->ini.environementQueryClient.serviceName << std::endl;
		status = COMP->environementQueryClient->connect(COMP->ini.environementQueryClient.serverName,
				COMP->ini.environementQueryClient.serviceName);
		while (status != CHS::SMART_OK) {
			usleep(500000);
			status = COMP->environementQueryClient->connect(COMP->ini.environementQueryClient.serverName,
					COMP->ini.environementQueryClient.serviceName);
