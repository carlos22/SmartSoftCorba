...
void ManipulatorEventClientHandler::handleEvent(const CHS::EventId id, const Smart::CommManipulatorEventResult &r) throw() {
	 ...
	 */
	if (r.get_event() == Smart::GOAL_REACHED) {
		flag = COMP->SEND_TRAJECTORY;
	} else if (r.get_event() == Smart::COLLISION || r.get_event() == Smart::NO_SOLUTION_FOUND || r.get_event()
			== Smart::VALUE_OUT_OF_RANGE) {
		flag = COMP->FAILURE;
	} else if (r.get_event() == Smart::GOAL_NOT_REACHED) {
		flag = COMP->NEUTRAL;
	} else {
		flag = COMP->FAILURE;
	}
