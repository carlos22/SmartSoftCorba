...
void PlayerPushTimedHandler::handlePushTimer(CHS::PushTimedServer<CommBasicObjects::CommBaseState> & server) throw()
{
    ...
    COMP->robotPos.set_base_alpha( COMP->piToPiRad(COMP->oldPos.get_base_alpha() + da ));

    COMP->rawPos.set_x( COMP->rawPos.get_x() + dx );
    COMP->rawPos.set_y( COMP->rawPos.get_y() + dy );
    COMP->rawPos.set_base_alpha( COMP->piToPiRad(COMP->rawPos.get_base_alpha() + da ));
...
PlayerPushTimedHandler::PlayerPushTimedHandler() {
    ...
    // initialize
    COMP->robotPos.set_x(0.0);
    COMP->robotPos.set_y(0.0);
    COMP->robotPos.set_base_alpha(0.0);

    COMP->rawPos.set_x(0.0);
    COMP->rawPos.set_y(0.0);
    COMP->rawPos.set_base_alpha(0.0);

    oldth = 0;
    COMP->totalDistance = 0;

    // initialize covariance matrix
    COMP->robotPos.set_cov(0,0, 50*50 );
    COMP->robotPos.set_cov(1,1, 50*50 );
    COMP->robotPos.set_cov(2,2, 5*5/180.0*M_PI );

    COMP->oldPos = COMP->robotPos;

    // Uncertainty of robot
    // TODO move values to ini file
    COMP->lamdaSigmaD = 50*50/1000.0;
    COMP->lamdaSigmaDeltaAlpha = (5*5/360.0) /180.0 * M_PI;
    COMP->lamdaSigmaDeltaBeta = (2*2/1000.0) /180.0 * M_PI;
}
