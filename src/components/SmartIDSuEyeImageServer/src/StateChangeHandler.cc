...
void StateChangeHandler::handleEnterState(const std::string & substate) throw()
{
	...
	{
		if (COMP->camera.openCamera() && COMP->camera.loadParameters(COMP->ini.hardware.parameter_file))
		{
	...
	{
		COMP->imageTask.createImageBuffer();
...
void StateChangeHandler::handleQuitState(const std::string & substate) throw()
{
	...
	if (substate == "nonneutral")
	{
		if (!COMP->camera.closeCamera())
		{
			std::cerr << "[ERROR] Could not close camera!" << std::endl;
		}
	}
