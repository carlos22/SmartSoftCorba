...
public:
	...
	operator CommManipulationPlannerObjectsIDL::CommManipulationPlannerParameter() const
	{
	...
	// 	 PARALLELIZATION_ON
	//   PARALLELIZATION_OFF
	//   GRASPING_SIMPLE ?lowerHeightBound, upperHeightBound, lowerDepthBound, upperDepthBound, lowerAngleBound, upperAngleBound, lowerRollBound, upperRollBound
	//   GRASPING_ADVANCED
	//   GRASPING_NONE
	...
	inline int set(std::string &inString) {
		...
		// --------------------
			...
			if (error == 0) {
				...
				// watch out, parameter08 is string!

				do {
					param = strsep(&input, LISP_SEPARATOR);
				} while ((param != NULL) && (strlen(param) == 0));

				if ((param != NULL) && (sscanf(param, "%lf", &idl_CommManipulationPlannerParameter.parameter09) == 1)) {
					// everything ok
				} else {
					error = -1;
				}
				...
				if ((param != NULL) && (sscanf(param, "%lf", &idl_CommManipulationPlannerParameter.parameter10) == 1)) {
					// everything ok
				} else {
					error = -1;
				}
	...
	inline void get(ManipulationPlannerParameterMode &tag, int32_t &value01, double &value02, double &value03, double &value04,
			double &value05, double &value06, double &value07, std::string &type, double &value09, double &value10) const {
		...
		value09 = idl_CommManipulationPlannerParameter.parameter09;
		value10 = idl_CommManipulationPlannerParameter.parameter10;
	}
